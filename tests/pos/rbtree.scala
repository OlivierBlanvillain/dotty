...
object RedBlackTree {
  ...
  def countInRange[A](tree: Tree[A, _], from: Option[A], to: Option[A])(implicit ordering: Ordering[A]): Int =
    ...
    (from, to) match {
      ...
      case (Some(lb: A), _) if ordering.lt(tree.key, lb) => countInRange(tree.right, from, to)
      // if node is greater than or equal to the upper bound, try the tree on the left, it might be in range
      case (_, Some(ub: A)) if ordering.gteq(tree.key, ub) => countInRange(tree.left, from, to)
  ...
  def rangeImpl[A: Ordering, B](tree: Tree[A, B], from: Option[A], until: Option[A]): Tree[A, B] = (from, until) match {
    case (Some(from: A), Some(until: A)) => this.range(tree, from, until)
    case (Some(from: A), None)        => this.from(tree, from)
    case (None,       Some(until: A)) => this.until(tree, until)
    case (None,       None)        => tree
  }
