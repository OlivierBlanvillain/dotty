...
//    javaTypeToValueTypeRef(jtype) = vcls
  ...
  lazy val PhantomClass: ClassSymbol = {
    val cls = completeClass(enterCompleteClassSymbol(ScalaPackageClass, tpnme.Phantom, NoInits | Trait, List(AnyType)))

    val any = enterCompleteClassSymbol(cls, tpnme.Any, Protected | Final | NoInits | Trait, Nil)
    val nothing = enterCompleteClassSymbol(cls, tpnme.Nothing, Protected | Final | NoInits | Trait, List(any.typeRef))
