...
trait ConstraintHandling {
  ...
  private def addOneBound(param: TypeParamRef, bound: Type, isUpper: Boolean): Boolean =
    !constraint.contains(param) || {
      def occursIn(bound: Type): Boolean = {
        ...
        (b eq param) || {
          b match {
            case AndType(tp1, tp2) => occursIn(tp1) || occursIn(tp2)
            case OrType(tp1, tp2)  => occursIn(tp1) || occursIn(tp2)
  ...
  def instanceType(param: TypeParamRef, fromBelow: Boolean): Type = {
    ...
    def isFullyDefined(tp: Type): Boolean = tp match {
      ...
      case AndType(tp1, tp2) => isFullyDefined(tp1) && isFullyDefined(tp2)
      case OrType (tp1, tp2) => isFullyDefined(tp1) && isFullyDefined(tp2)
      case _ => true
    }
