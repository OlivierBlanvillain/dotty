...
trait TypeAssigner {
  ...
  protected def normalizedType(tp: Type)(implicit ctx: Context): Type = {
    def skipNormalization = {
      true || ctx.isAfterTyper || ctx.mode.is(Mode.Type) || ctx.mode.is(Mode.InferringReturnType) ||
    ...
    val ownType = fn.tpe.widen match {
      case fntpe: MethodType =>
        if (sameLength(fntpe.paramInfos, args) || ctx.phase.prev.relaxedTyping) {
          ...
          val tpe1 =
            if (/*!ctx.isAfterTyper &&*/ (fn.symbol.isTransparentMethod || ctx.mode.is(Mode.Transparent))) TypeOf(tpe, tree)
            else tpe
          normalizedType(tpe1)
        } else
  ...
  def assignType(tree: untpd.If, thenp: Tree, elsep: Tree)(implicit ctx: Context) = {
    ...
    if (/*!ctx.isAfterTyper &&*/ ctx.mode.is(Mode.Transparent))
