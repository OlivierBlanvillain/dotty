...
object CollectEntryPoints{
  ...
  def isJavaEntryPoint(sym: Symbol)(implicit ctx: Context): Boolean = {
    ...
    def hasJavaMainMethod(sym: Symbol): Boolean =
      (sym.denot.info member nme.main).alternatives exists(x => isJavaMainMethod(x.symbol))

    def fail(msg: String, pos: Position = sym.pos) = {
      ctx.warning(          sym.name +
        s" has a main method with parameter type Array[String], but ${sym.denot.fullName} will not be a runnable program.\n  Reason: $msg",
        sym.pos
    ...
    val possibles = if (sym.flags is Flags.Module) (sym.denot.info nonPrivateMember nme.main).alternatives else Nil
    ...
    hasApproximate && {
      // Before erasure so we can identify generic mains.
      {
        ...
        else if (companion.denot.info.member(nme.main) != NoDenotation)
