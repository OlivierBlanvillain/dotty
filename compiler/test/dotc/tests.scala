...
//    "-Yshow-suppressed-errors",
  ...
  implicit val defaultOptions: List[String] = noCheckOptions ++ {
    if (isRunByJenkins) List("-Xprint:all", " -Ycheck:tailrec,resolveSuper,mixin,restoreScopes,labelDef") // should be Ycheck:all, but #725
    else List("-Xprint:all", "-Ycheck:tailrec,resolveSuper,mixin,restoreScopes,labelDef")
  ...
  def pickle_pickleOK = compileFiles(testsDir + "pickling/", testPickling)
// This directory doesn't exist anymore
// def pickle_pickling = compileDir(coreDir, "pickling", testPickling)
  def pickle_ast = compileDir(dotcDir, "ast", testPickling)
  def pickle_inf = compileFile(posDir, "pickleinf", testPickling)

  //def pickle_core = compileDir(dotcDir, "core", testPickling, xerrors = 2) // two spurious comparison errors in Types and TypeOps

  def pos_arraycopy =
    compileFile(runDir, "arraycopy", List("-Ylog-classpath"))
  def pos_t2168_pat = compileFile(posDir, "t2168", twice)
  def pos_erasure = compileFile(posDir, "erasure", twice)
  def pos_Coder() = compileFile(posDir, "Coder", twice)
  def pos_blockescapes() = compileFile(posDir, "blockescapes", twice)
  def pos_collections() = compileFile(posDir, "collections", twice)
  def pos_functions1() = compileFile(posDir, "functions1", twice)
  def pos_implicits1() = compileFile(posDir, "implicits1", twice)
  def pos_inferred() = compileFile(posDir, "inferred", twice)
  def pos_Patterns() = compileFile(posDir, "Patterns", twice)
  def pos_selftypes() = compileFile(posDir, "selftypes", twice)
  def pos_varargs() = compileFile(posDir, "varargs", twice)
  def pos_vararg_patterns() = compileFile(posDir, "vararg-pattern", twice)
  def pos_opassign() = compileFile(posDir, "opassign", twice)
  def pos_typedapply() = compileFile(posDir, "typedapply", twice)
  def pos_nameddefaults() = compileFile(posDir, "nameddefaults", twice)
  def pos_desugar() = compileFile(posDir, "desugar", twice)
  def pos_sigs() = compileFile(posDir, "sigs", twice)
  def pos_typers() = compileFile(posDir, "typers", twice)
  def pos_typedIdents() = compileDir(posDir, "typedIdents", twice)
  def pos_assignments() = compileFile(posDir, "assignments", twice)
  def pos_packageobject() = compileFile(posDir, "packageobject", twice)
  def pos_overloaded() = compileFile(posDir, "overloaded", twice)
  def pos_overrides() = compileFile(posDir, "overrides", twice)
  def pos_javaOverride() = compileDir(posDir, "java-override", twice)
  def pos_templateParents() = compileFile(posDir, "templateParents", twice)
  def pos_overloadedAccess = compileFile(posDir, "overloadedAccess", twice)
  def pos_approximateUnion = compileFile(posDir, "approximateUnion", twice)
  def pos_tailcall = compileDir(posDir, "tailcall", twice)
  def pos_valueclasses = compileFiles(posDir + "pos_valueclasses/", twice)
  def pos_nullarify = compileFile(posDir, "nullarify", args = "-Ycheck:nullarify" :: Nil)
  def pos_subtyping = compileFile(posDir, "subtyping", twice)
  def pos_packageObj = compileFile(posDir, "i0239", twice)
  def pos_anonClassSubtyping = compileFile(posDir, "anonClassSubtyping", twice)
  ...
  def rewrites = compileFile(posScala2Dir, "rewrites", "-rewrite" :: scala2mode)

  def pos_t8146a = compileFile(posSpecialDir, "t8146a")(allowDeepSubtypes)
  ...
  def pos_utf8 = compileFile(posSpecialDir, "utf8encoded", explicitUTF8)
  def pos_utf16 = compileFile(posSpecialDir, "utf16encoded", explicitUTF16)

  def new_all = compileFiles(newDir, twice)
  // def repl_all = replFiles(replDir) ø

  def neg_all = compileFiles(negDir, verbose = true, compileSubDirs = false)
  def neg_typedIdents() = compileDir(negDir, "typedIdents")

  val negCustomArgs = negDir + "customArgs/"

  def neg_typers() = compileFile(negCustomArgs, "typers")(allowDoubleBindings)
  def neg_overrideClass = compileFile(negCustomArgs, "overrideClass", scala2mode)
  def neg_autoTupling = compileFile(negCustomArgs, "autoTuplingTest", args = "-language:noAutoTupling" :: Nil)
  def neg_i1050 = compileFile(negCustomArgs, "i1050", List("-strict"))
  def neg_i1240 = compileFile(negCustomArgs, "i1240")(allowDoubleBindings)
  def neg_i2002 = compileFile(negCustomArgs, "i2002")(allowDoubleBindings)

  val negTailcallDir = negDir + "tailcall/"
  def neg_tailcall_t1672b = compileFile(negTailcallDir, "t1672b")
  def neg_tailcall_t3275 = compileFile(negTailcallDir, "t3275")
  def neg_tailcall_t6574 = compileFile(negTailcallDir, "t6574")
  def neg_tailcall = compileFile(negTailcallDir, "tailrec")
  def neg_tailcall2 = compileFile(negTailcallDir, "tailrec-2")
  def neg_tailcall3 = compileFile(negTailcallDir, "tailrec-3")

  def neg_nopredef = compileFile(negCustomArgs, "nopredef", List("-Yno-predef"))
  ...
  // def run_tuples = runFiles(testsDir + "tuples/")
  ...
  def compileStdLib = compileList("compileStdLib", stdlibFiles, "-migration" :: "-Yno-inline" :: scala2mode)
  def compileMixed = compileLine(
      """../tests/pos/B.scala
  ...
  def compileIndexedSeq = compileLine("../scala-scala/src/library/scala/collection/immutable/IndexedSeq.scala")
  // def compileParSetLike = compileLine("../scala-scala/src/library/scala/collection/parallel/mutable/ParSetLike.scala") ø
  // def compileParSetSubset = compileLine(
  ...
  //       |../scala-scala/src/library/scala/collection/mutable/SetLike.scala""".stripMargin)(scala2mode ++ defaultOptions) ø
  ...
  def dotc_ast = compileDir(dotcDir, "ast")
  def dotc_config = compileDir(dotcDir, "config")
  def dotc_core = compileDir(dotcDir, "core")(allowDeepSubtypes)// twice omitted to make tests run faster
  def dotc_core_nocheck = compileDir(dotcDir, "core")(noCheckOptions ++ classPath)

// This directory doesn't exist anymore
//  def dotc_core_pickling = compileDir(coreDir, "pickling")(allowDeepSubtypes)// twice omitted to make tests run faster

  def dotc_transform = compileDir(dotcDir, "transform")(allowDeepSubtypes)// twice omitted to make tests run faster

  def dotc_parsing = compileDir(dotcDir, "parsing") // twice omitted to make tests run faster

  def dotc_printing = compileDir(dotcDir, "printing") // twice omitted to make tests run faster

  def dotc_reporting = compileDir(dotcDir, "reporting") // twice omitted to make tests run faster

  def dotc_typer = compileDir(dotcDir, "typer")// twice omitted to make tests run faster
  ...
  def dotc_util = compileDir(dotcDir, "util") // twice omitted to make tests run faster

  def tools_io = compileDir(toolsDir, "io") // inner class has symbol <none>

  def helloWorld = compileFile(posDir, "HelloWorld")
  def labels = compileFile(posDir, "Labels", twice)
  //def tools = compileDir(dottyDir, "tools", "-deep" :: Nil)(allowDeepSubtypes)

  def testNonCyclic = compileList("testNonCyclic", List(
      dotcDir + "CompilationUnit.scala",
      coreDir + "Types.scala",
      dotcDir + "ast/Trees.scala"
    ), List("-Xprompt") ++ staleSymbolError ++ twice)

  def testIssue_34 = compileList("testIssue_34", List(
  ...
  //def dotc_compilercommand = compileFile(dotcDir + "config/", "CompilerCommand")

  //TASTY tests
  def tasty_new_all = compileFiles(newDir, testPickling)

  def tasty_dotty = compileDir(sourceDir, "dotty", testPickling)

  // Disabled because we get stale symbol errors on the SourceFile annotation, which is normal.
  // def tasty_annotation_internal = compileDir(s"${dottyDir}annotation/", "internal", testPickling)

  def tasty_runtime = compileDir(s"${libDir}dotty/", "runtime", testPickling)
  def tasty_runtime_vc = compileDir(s"${libDir}dotty/runtime/", "vc", testPickling)

  def tasty_tools = compileDir(dottyDir, "tools", testPickling)

  //TODO: issue with ./src/dotty/tools/backend/jvm/DottyBackendInterface.scala
  def tasty_backend_jvm = compileList("tasty_backend_jvm", List(
    "CollectEntryPoints.scala", "GenBCode.scala", "LabelDefs.scala",
    "scalaPrimitives.scala"
  ) map (s"${backendDir}jvm/" + _), testPickling)

  //def tasty_backend_sjs = compileDir(s"${backendDir}", "sjs", testPickling)

  def tasty_dotc = compileDir(toolsDir, "dotc", testPickling)
  def tasty_dotc_ast = compileDir(dotcDir, "ast", testPickling)
  def tasty_dotc_config = compileDir(dotcDir, "config", testPickling)

  //TODO: issue with ./src/dotty/tools/dotc/core/Types.scala
  def tasty_core = compileList("tasty_core", List(
  ...
  def tasty_classfile = compileDir(coreDir, "classfile", testPickling)
  def tasty_tasty = compileDir(coreDir, "tasty", testPickling)
  def tasty_unpickleScala2 = compileDir(coreDir, "unpickleScala2", testPickling)

  //TODO: issue with ./src/dotty/tools/dotc/parsing/Parsers.scala
  def tasty_dotc_parsing = compileList("tasty_dotc_parsing", List(
  ...
  def tasty_dotc_printing = compileDir(dotcDir, "printing", testPickling)

  def tasty_dotc_repl = compileDir(dotcDir, "repl", testPickling)

  //def tasty_dotc_reporting = compileDir(dotcDir, "reporting", testPickling)
  def tasty_dotc_rewrite = compileDir(dotcDir, "rewrite", testPickling)

  //TODO: issues with LazyVals.scala, PatternMatcher.scala
  def tasty_dotc_transform = compileList("tasty_dotc_transform", List(
  ...
  def tasty_typer = compileList("tasty_typer", List(
  ...
  def tasty_dotc_util = compileDir(dotcDir, "util", testPickling)
  def tasty_tools_io = compileDir(toolsDir, "io", testPickling)
