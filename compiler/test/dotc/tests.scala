...
// def pickle_pickling = compileDir(coreDir, "pickling", testPickling)
  ...
  def pos_859 = compileFile(posSpecialDir, "i859", scala2mode)(allowDeepSubtypes)
  ...
  // def repl_all = replFiles(replDir)
  ...
  def neg_cli_error = compileFile(negCustomArgs, "cliError", List("-thisOptionDoesNotExist"))
  ...
  @Test def run_all = runFiles(testsDir + "run/tuples/")
...
//  def dotc_core_pickling = compileDir(coreDir, "pickling")(allowDeepSubtypes)// twice omitted to make tests run faster
  ...
  def tasty_tests = compileDir(testsDir, "tasty", testPickling)
