...
object TupleCons {
  def apply[H, T <: Tuple](h: H, t: T): TupleCons[H, T] =
    ((t: Any) match {
      ...
      // seq 2 21 | while read i; do
      //   echo -n "case t: scala.Tuple$i["
      //   seq $i | while read l; do echo -n "_, "; done
      //   echo -n "] => new scala.Tuple$(expr $i + 1)(h, "
      //   seq $i | while read l; do echo -n "t._$l, "; done
      //   echo ")"
  ...
  def unapply[H, T <: Tuple](t: TupleCons[H, T]): Pair[H, T] =
    ((t: Any) match {
      case impln: LargeTuple[_, _] =>
        ...
        val tail =
          if (s == 23) {
            // seq 22 | xargs -i -n1 echo "underlying({}), " | xargs
            new scala.Tuple22(underlying(1), underlying(2), underlying(3), underlying(4), underlying(5), underlying(6), underlying(7), underlying(8), underlying(9), underlying(10), underlying(11), underlying(12), underlying(13), underlying(14), underlying(15), underlying(16), underlying(17), underlying(18), underlying(19), underlying(20), underlying(21), underlying(22)).asInstanceOf[T]
          } else {
            s = s - 1
      ...
      // seq 21 | while read i; do
      //   echo -n "case t: scala.Tuple$(expr $i + 1)["
      //   seq $(expr $i + 1) | while read l; do echo -n "_, "; done
      //   echo -n "] => new Pair(t._1, new scala.Tuple$i("
      //   seq 2 $(expr $i + 1) | while read l; do echo -n "t._$l, "; done
      //   echo "))"
      // done | sed -e "s/, )/)/g" -e "s/, ]/]/g"
...
//   echo "trait  DottyTuple$i"
//   echo "object DottyTuple$i {"
//   echo -n "  def apply["
//   seq $i | while read l; do echo -n "T$l, "; done
//   echo -n "]("
//   seq $i | while read l; do echo -n "_$l: T$l, "; done
//   echo -n "): ("
//   seq $i | while read l; do echo -n "T$l, "; done
//   echo ") = ???"
//   echo -n "  def unapply["
